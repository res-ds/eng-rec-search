...
def _generate_response(open_ai_client: AzureOpenAI, prompt: str) -> str:
...
def _augment_prompt(prompt: str, similar_issues: list[SimilarIssue]) -> str:
    ...
    You are given a current issue symptoms and previous issues with their symptoms, recommendations and resolutions as information.
    Generate a reasonable recomandation on what to do to resolve the current issue in a very succint way (max. 50 words).
...
def get_suggested_recommendation(open_ai_client: AzureOpenAI, prompt: str, similar_issues: list[SimilarIssue]) -> str:
